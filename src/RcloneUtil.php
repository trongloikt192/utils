...
class RcloneUtil
{
    ...
    {
        $cmd = sprintf('rclone copy %s %s:%s --ignore-existing --ignore-checksum --transfers 4 --checkers 8 --onedrive-chunk-size 160M --drive-chunk-size 128M --log-level INFO --log-file /rclone/rclone.log 2>&1', $sourcePath, $this->entity->rclone_name, $toFolderPath);
        $out = shell_exec($cmd);
        if (strlen(trim($out)) > 0) {
            throw new UtilException($out);
        }
